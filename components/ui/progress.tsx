...
export function ProgressBar({ progress, className, barCalssName }: { progress: number, className?: string, barCalssName?: string }) {
    return (
        <div className={cn("w-full bg-fg/5 rounded-sm h-4 overflow-hidden", className)}>
            <div
                className={cn("h-4 rounded-sm transition-all duration-500 ease-in-out", barCalssName)}
                style={{
